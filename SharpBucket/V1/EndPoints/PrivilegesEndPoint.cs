...
namespace PortableBitBucketClient.V1.EndPoints
{
	...
	public class PrivilegesEndPoint
	{
		...
		public PrivilegesEndPoint(string accountName, BitBucketClientV1 sharpBucketV1)
		{
		...
		/// <returns></returns>
		public async Task<List<RepositoryPrivileges>> ListRepositoryPrivileges(string repository)
		{
			var overrideUrl = _baseUrl + repository + "/";
			return await _sharpBucketV1.Get(new List<RepositoryPrivileges>(), overrideUrl);
		}
		...
		/// <returns></returns>
		public async Task<RepositoryPrivilegesUser> GetPrivilegesForAccount(string repository, string accountName)
		{
			var overrideUrl = _baseUrl + repository + "/" + accountName;
			return await _sharpBucketV1.Get(new RepositoryPrivilegesUser(), overrideUrl);
		}
	}
}